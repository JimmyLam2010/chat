...
class DB
{
  ...
  {
    $this->sql = "UPDATE {$this->pre}$table ";
    return $this;
  }

  public function set($coloum)
  {
    if(is_array($coloum))
    {
      $this->sql .= "SET ";
      foreach($coloum as $key => $item){
        $this->sql .= "{$key}='$item', " ;
      }
  ...
  public function insert($table, $coloum)
  {
    $coloums = "";
    $values = "";
    if(is_array($coloum)){
      ...
      $coloums = trim($coloums,", ");
      $values = trim($values,", ");
    }
    
    
    $this->sql = "INSERT INTO {$this->pre}$table ($coloums) VALUES($values)";
    return $this;
  }

  public function count($table)
  {
    $this->sql = "SELECT count(*) as count from {$this->pre}$table";
    $res = mysqli_query($this->link,$this->sql);
  ...
  public function deleteAll($table, $values, $coloum)
  {
    $str = implode(', ',$values);
    $this->sql = "DELETE FROM {$this->pre}$table WHERE $coloum IN ({$str})";
